// ============================================================
// community_membership_saas v2 — Course / Lesson Access Evaluation
// ============================================================
// コースおよびレッスンのアクセス可否判定ロジック。
...
//      - rules_based  → course_access_rules の OR 評価
...
//
// レッスンアクセス評価フロー:
//   1. drip_days: membership.joined_at + drip_days <= now
//   2. unlock_level: member_points.level >= unlock_level
//   いずれか不足 → アクセス不可 (理由と解放日/必要レベルを返却)
// ============================================================
...
export type LessonAccessCheckResult = {
  allowed: boolean;
  reason: string;
  unlock_date?: string;
...
export async function checkCourseAccess(params: {
  courseId: string;
  tenantId: string;
  userId: string | null;
  ...
  // 5. rules_based → OR 評価
  if (mode === "rules_based") {
    return evaluateRules(params.courseId, params.tenantId, params.userId);
  }
...
export async function checkLessonAccess(params: {
...
}): Promise<LessonAccessCheckResult> {
  const supabase = createAdminClient();

  // レッスン取得
  const { data: lesson, error } = await supabase
    .from("course_lessons")
    .select("id, drip_days, unlock_level, is_preview")
  ...
  // drip_days チェック
  if (lesson.drip_days !== null && lesson.drip_days > 0) {
    const joinedAt = new Date(params.membershipJoinedAt);
  ...
  // unlock_level チェック
  if (lesson.unlock_level !== null && lesson.unlock_level > 0) {
    ...
    const currentLevel = memberPoints?.level ?? 1;

    if (currentLevel < lesson.unlock_level) {
      return {
        allowed: false,
        reason: "level_locked",
        required_level: lesson.unlock_level,
      };
    }
  }

  return { allowed: true, reason: "accessible" };
}
...
async function evaluateRules(
  ...
  for (const rule of rules) {
    ...
    if (rule.rule_type === "purchase_based") {
      const match = await checkPurchaseAccess(tenantId, userId, courseId);
