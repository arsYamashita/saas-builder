...
describe("POST /api/generation-runs/[runId]/review-step — error-leak wiring", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("does not leak the DB error when the update fails (serverErrorResponse path)", async () => {
    ...
    const res = await POST(
      makeRequest({ stepKey: "implementation", action: "approved" }),
      { params }
    );
    ...
    expect(json.error).toBe("Failed to update step review");
  ...
  it("does not leak an unexpected thrown error from requireRunAccess", async () => {
    ...
    assertNoLeak(text, FORBIDDEN);
    const json = JSON.parse(text);
    expect(json.error).toBe("Failed to review step");
  });
});
