...
describe("POST /api/projects/[projectId]/split-run-to-files — error-leak wiring", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("does not leak the DB error surfaced by getLatestImplementationRun()", async () => {
    mockGetLatestImplementationRun.mockRejectedValue(
      new Error(
        'Failed to fetch implementation run: relation "implementation_runs_secret_cache" does not exist'
      )
    );
    ...
    assertNoLeak(text, ["implementation_runs_secret_cache", "does not exist"]);
