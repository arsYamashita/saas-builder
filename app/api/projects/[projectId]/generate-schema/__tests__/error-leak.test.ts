...
describe("POST /api/projects/[projectId]/generate-schema — error-leak wiring", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("does not leak the DB error when the blueprint lookup fails (serverErrorResponse path)", async () => {
    mockGetLatestBlueprintByProjectId.mockRejectedValue(
      new Error(
        'duplicate key value violates unique constraint "blueprints_pkey", code 23505'
      )
    );
    ...
    assertNoLeak(text, ["blueprints_pkey", "duplicate key", "23505"]);
    const json = JSON.parse(text);
    expect(json.error).toBe("Failed to generate schema");
