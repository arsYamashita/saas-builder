/**
 * Thin wrapper around the vault-hosted canonical `kb:reconcile` script.
 *
 * 2026-07-06: extracted to
 * `~/Documents/my-vault/_scripts/kb_checklist/kb-reconcile.mjs` (plain,
 * dependency-free Node ESM) alongside the checklist generator -- see
 * scripts/generate-error-checklist.ts for the rationale.
 *
 * Usage (unchanged):
 *   npm run kb:reconcile
 *   npm run kb:reconcile -- --dry-run
 *   npm run kb:reconcile -- --project my-app
 *   VAULT_PATH=/custom/path npm run kb:reconcile
 *
 * `scripts/kb-reconcile-core.ts` (the pure trailer-parsing logic,
 * unit-tested by scripts/__tests__/kb-reconcile-core.test.ts) stays in
 * this repo, behaviorally identical to the vault copy, for the same
...
async function main(): Promise<void> {
  ...
  const scriptPath = path.join(vaultPath, "_scripts", "kb_checklist", "kb-reconcile.mjs");

  try {
    await import(pathToFileURL(scriptPath).href);
  } catch (err) {
    console.error(
      `[kb:reconcile] could not load the vault script at "${scriptPath}": ` +
