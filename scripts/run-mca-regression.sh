...
# membership_content_affiliate regression test runner
...
for i in $(seq 1 $MAX_POLLS); do
  ...
  GEN_STATUS=$(echo "$PROJ_RESP" | jq -r '.project.latestGenerationRun.overall_status // "unknown"')

  if [ "$GEN_STATUS" = "completed" ] || [ "$GEN_STATUS" = "failed" ]; then
    break
  fi

  CURRENT_STEP=$(echo "$PROJ_RESP" | jq -r '.project.latestGenerationRun.current_step // "unknown"')
...
echo "$PROJ_RESP" | jq -r '
  .project.latestGenerationRun // {} |
  "  blueprint:      \(.step_blueprint // "N/A")\n  implementation:  \(.step_implementation // "N/A")\n  schema:          \(.step_schema // "N/A")\n  api_design:      \(.step_api_design // "N/A")\n  split_files:     \(.step_split_files // "N/A")\n  export_files:    \(.step_export_files // "N/A")"
'
echo ""

echo "--- Quality Run ---"
echo "$PROJ_RESP" | jq -r '
  .project.latestQualityRun // {} |
  "  status:      \(.status // "N/A")\n  lint:        \(.lint_status // "N/A")\n  typecheck:   \(.typecheck_status // "N/A")\n  playwright:  \(.playwright_status // "N/A")"
'
echo ""

echo "--- Saved Counts ---"
BLUEPRINTS=$(echo "$PROJ_RESP" | jq '.project.blueprintsCount // 0')
IMPL_RUNS=$(echo "$PROJ_RESP" | jq '.project.implementationRunsCount // 0')
GEN_FILES=$(echo "$PROJ_RESP" | jq '.project.generatedFilesCount // 0')
...
# --- Summary ---
QUALITY_STATUS=$(echo "$PROJ_RESP" | jq -r '.project.latestQualityRun.status // "unknown"')
LINT=$(echo "$PROJ_RESP" | jq -r '.project.latestQualityRun.lint_status // "unknown"')
TYPECHECK=$(echo "$PROJ_RESP" | jq -r '.project.latestQualityRun.typecheck_status // "unknown"')
PLAYWRIGHT=$(echo "$PROJ_RESP" | jq -r '.project.latestQualityRun.playwright_status // "unknown"')

echo "=== REGRESSION SUMMARY ==="
if [ "$GEN_STATUS" = "completed" ] && [ "$LINT" = "pass" ] && [ "$TYPECHECK" = "pass" ] && [ "$PLAYWRIGHT" = "pass" ]; then
  echo "RESULT: GREEN"
else
