  ...
  if (!result.ok) {
    ...
    console.error(
      "[preflight-env] Upstash Redis (UPSTASH_REDIS_REST_URL / UPSTASH_REDIS_REST_TOKEN) must be " +
        ...
        "runtime (lib/rate-limit.ts denies requests rather than silently allowing them) — " +
        "meaning login/signup/AI-generation routes would serve 429s to real traffic. " +
